...
class Informer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # Decoder
        DecAttn = FullAttention if self.transformer_dec else Attn
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer(
                        DecAttn(True, factor, attention_dropout=dropout, output_attention=False),
                        d_model,
                        n_heads,
                        mix=mix,
                    ),
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        ...
        if self.transformer_dec:
            x_dec = x_dec[:, self.label_len - 1 : -1, :]
            x_mark_dec = x_mark_dec[:, self.label_len - 1 : -1, :]
            if self.features == "MS":
            ...
            if not self.training:
                ...
                for i in range(self.pred_len):
                    # x_mark_dec: (batch_size, label_len + pred_len, time_num)
                    dec_out = self.dec_embedding(x_dec, x_mark_dec[:, [i], :])
                    dec_out = self.decoder(
                        dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask, transformer_dec=True
                    )
